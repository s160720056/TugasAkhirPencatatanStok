...
// CREATE INDEX idx_transaksi_tipe_tanggal ON transaksi(tipe_transaksi, tanggal_transaksi);
        ApplyDatabase::applyToMainAndDynamicDatabases(function ($db) {
            Schema::connection($db)->table('transaksi', function (Blueprint $table) {
                $table->index(['id_barang', 'tanggal_transaksi'], 'idx_transaksi_id_barang_tanggal');
                $table->index(['tipe_transaksi', 'tanggal_transaksi'], 'idx_transaksi_tipe_tanggal');
            });
        });
    ...
    public function down(): void
    {
        ApplyDatabase::applyToMainAndDynamicDatabases(function ($db) {
            Schema::connection($db)->table('transaksi', function (Blueprint $table) {
                //drop index
                $table->dropIndex('idx_transaksi_id_barang_tanggal');
                $table->dropIndex('idx_transaksi_tipe_tanggal');
            });
        });
    }
};
