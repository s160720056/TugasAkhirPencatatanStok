@extends('layouts.app', ['menu' => 'buku kas'])
@section('content')
    @php
   ...
   <div class="flip-book" id="book">
        ...
        <div class="page">
            ...
            <table>
                ...
                @foreach ($barangList->chunk(20) as $chunk)
                    @foreach ($chunk as $index => $b)
                    <tr>
                        <td>{{ $index + 1 }}</td>
                        <td>{{ $b->kode_barang }}</td>
                        <td>{{ $b->nama_barang }}</td>
        ...
        @if ($months->isNotEmpty())
            ...
            <!-- Halaman pertama bulan -->
            <div class="page" data-month="{{ $firstMonth }}">
                <h3 style="text-align:center">
                    {{ Carbon::parse($firstMonth.'-01')->translatedFormat('F Y') }} - Masuk
                </h3>
                @include('page.bukuStok.partials.table_masuk')
            </div>

            <div class="page" data-month="{{ $firstMonth }}">
                <h3 style="text-align:center">
                    {{ Carbon::parse($firstMonth.'-01')->translatedFormat('F Y') }} - Keluar & Rekap
                </h3>
                @include('page.bukuStok.partials.table_keluar')
            </div>
        @endif
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            const loadedMonths = new Set(['{{ $firstMonth ?? '' }}']); // Initially loaded month

            pageFlip.loadFromHTML(document.querySelectorAll(".page"));
            pageFlip.update();

            // Hide original page elements once the book is initialized so they don't stack vertically.
            document.querySelectorAll("#book .page").forEach((el) => {
                el.style.display = "none";
            });

            const totalPages = Math.max(0, pageFlip.getPageCount() - 1);
            const currentSpan = document.querySelector(".page-current");

            const setCurrent = (pageIndex) => {
                if (pageIndex === 0) {
                    currentSpan.innerText = 'Cover';
                } else {
                    currentSpan.innerText = pageIndex;
                }
            };

            document.querySelector(".page-total").innerText = totalPages;
            setCurrent(0);
            ...
            const loadMonth = async (month) => {
                ...
                try {
                    const response = await fetch(`/transaksiFlipBook/month/${month}`)
                    if (!response.ok) throw new Error('Network response was not ok');
                    const html = await response.text();

                    // Append new pages to the book
                    const book = document.getElementById("book");
                    ...
                    // Update total pages
                    const newTotal = Math.max(0, pageFlip.getPageCount() - 1);
                    document.querySelector(".page-total").innerText = newTotal;
            ...
            pageFlip.on("flip", (e) => {
                ...
                // jika halaman sekarang adalah 2 halaman sebelum akhir
                if (currentPage === totalPages - 2) {
                    //asc
                    // const loadedCount = loadedMonths.size;
                    // if (loadedCount < allMonths.length) {
                    //     const nextMonth = allMonths[loadedCount];
                    //     loadMonth(nextMonth);
                    // }
